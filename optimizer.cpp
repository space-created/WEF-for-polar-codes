...
int calcACount = 0;
int computeWEFCountNaive = 0;
int computeWEFCount = 0;
...
vector<ll> multiply2(vector<ll>::iterator a, vector<ll>::iterator b, int size_a, int size_b) {
    vector<ll> prod(size_a * 2 - 1, 0);
    int ii = 0;
    for (vector<ll>::iterator i=a; i < (a + size_a); i++) {
        int jj = 0;
        for (vector<ll>::iterator j=b; j < (b + size_b); j++) {
            prod[ii+jj] += (*i) * (*j);
            jj++;
        }
        ii++;
...
void do_xor(vector <u8>& first_v, vector <u8>& second_v) {
...
pair<vector<ll>, vector<ll> > calcA(int n, vector <u8> u) {
    ...
    if (i % 2 == 0) {
        ...
        return {add(multiply2(f.first.begin(), g.first.begin(), f.first.size(), g.first.size()),
                    multiply2(f.second.begin(), g.second.begin(), f.second.size(), g.second.size())),
                add(multiply2(f.first.begin(), g.second.begin(), f.first.size(), g.second.size()),
                    multiply2(f.second.begin(), g.first.begin(), f.second.size(), g.second.size()))};
    } else {
        ...
        pair<vector<ll>, vector<ll> > g =  calcA(n / 2, even_odd_u.second);
        if (u.at(u.size() - 1) == 0) {
            return {multiply2(f.first.begin(), g.first.begin(), f.first.size(), g.first.size()),
                    multiply2(f.second.begin(), g.second.begin(), f.second.size(), g.second.size())};
        } else {
            return {multiply2(f.first.begin(), g.second.begin(), f.first.size(), g.second.size()),
                    multiply2(f.second.begin(), g.first.begin(), f.second.size(), g.first.size())};
        }
    }
}
...
void apply_random_constraints(vector<pair<bool, vector<int> > >& dynamic_constraints, vector<int> frozen_bits,
                              int version, vector<int> rows_to_add, int read_matrix) {
...
//                }
        ...
        for (int i = 0; i < dynamic_constraints.size(); ++i) {
            if (dynamic_constraints.at(i).second.empty()) {
                if (version) {
                    uniform_int_distribution<> dis(0, info_set_for_frozen_bit.at(i).size());

                    int random_number = dis(generator);
                    //            cout << random_number << '\n';

                    vector<int> vec_to_shuffle = info_set_for_frozen_bit.at(i);
                    shuffle(vec_to_shuffle.begin(), vec_to_shuffle.end(), generator);

                    dynamic_constraints.at(i).first = 1;
                    for (int j = 0; j < random_number; ++j) {
                        dynamic_constraints.at(i).second.push_back(vec_to_shuffle.at(j));
                    }
                    sort(dynamic_constraints.at(i).second.begin(), dynamic_constraints.at(i).second.end());
                } else {
                    vector<int> odds;
                    int is_not_zero = generate_random_bool_vector(odds, generator, info_set_for_frozen_bit.at(i).size());
                    if (is_not_zero) {
                        dynamic_constraints.at(i).first = 1;
                        for (int j = 0; j < odds.size(); ++j) {
                            if (odds.at(j) == 1) {
                                dynamic_constraints.at(i).second.push_back(info_set_for_frozen_bit.at(i).at(j));
                            }
                        }
                    }
                }
...
//    freopen("output.txt", "w", stdout);
    ...
    int m, n, k, last_frozen_pos = 0;
    cin >> version; // version of the random algorithm implementation
    cin >> iteration_number; // number of iterations
    cin >> read_matrix; // use bch matrix as base or not
    ...
    for (int iter = 0; iter < iteration_number; ++iter) {
        ...
        apply_random_constraints(dynamic_constraints, frozen_bits, version, rows_to_add, read_matrix);
        cout << "\ndynamic_constraints:\n";
        for (int i = 0; i < dynamic_constraints.size(); ++i) {
...
//    cout << "Time: " << duration.count() << '\n';
    ...
    cout << "CalcA: " << calcACount << '\n';
    cout << "computeWEFCountNaive: " << computeWEFCountNaive << '\n';
    cout << "computeWEFCount: " << computeWEFCount << '\n';
	return 0;
}
